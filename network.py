...
class Ranking(nn.Module):
    def __init__(self, max_user=1000000, max_item=10000, max_category=1000):
        super().__init__()
        self.user_embedding = nn.Embedding(max_user, 16)
        self.item_embedding = nn.Embedding(max_item, 12)
        self.category_embedding = nn.Embedding(max_category, 4)
        self.encoder = nn.GRU(input_size=16, hidden_size=16, num_layers=1, batch_first=True)
        self.attention = nn.MultiheadAttention(16, 1, batch_first=True)
